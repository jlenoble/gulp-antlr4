...
exports.default = function (options) {
  ...
  return _through2.default.obj(function (file, encoding, callback) {
    ...
    if (file.isBuffer()) {
      switch (_path2.default.extname(file.path).toLowerCase()) {
        case '.g4':
    ...
    if (mustRequireAfresh) {
      if (dataFiles.length > 0) {
        callback(new _gulpUtil.PluginError(PLUGIN_NAME, 'Cannot require afresh yet'));
      } else {
        callback(null);
      }
    } else {
          ...
          dataFiles.forEach(function (file) {
            try {
              (0, _consumeData2.default)({
                data: file.contents.toString('utf8'), ANTLR4: ANTLR4, mode: mode
              });
              _this.push(file);
            } catch (err) {
