...
exports.default = function (_options) {
  ...
  return _through2.default.obj(function (file, encoding, callback) {
    ...
    if (file.isStream()) {
      this.emit('error', new _gulpUtil2.PluginError(PLUGIN_NAME, 'Streams not supported (yet)!'));
      ...
      switch (_path2.default.extname(inputFile)) {
        case '.g4':
          ...
          return (0, _childProcessData2.default)((0, _child_process.spawn)('java', args)).then(function () {
            ...
            _this.emit('error', new _gulpUtil2.PluginError(PLUGIN_NAME, err));
            callback(null, file);
          });
            ...
            switch (mode) {
              ...
              case 'listener':
                try {
                  ...
                  this.emit('error', new _gulpUtil2.PluginError(PLUGIN_NAME, err));
                }
                break;
...
var _gulpUtil2 = require('gulp-util');
...
function checkJava() {
  ...
  if (!CLASSPATH) {
    throw new _gulpUtil2.PluginError(PLUGIN_NAME, new ReferenceError('Environment variable CLASSPATH is not defined;\nJava is not installed or is improperly set'));
  }
  ...
  if (matchJar === null) {
    throw new _gulpUtil2.PluginError(PLUGIN_NAME, new ReferenceError('Cannot find ANTLR4 .jar file;\nIt should appear in your CLASSPATH'));
  }
}
...
function getParserDir(options) {
  ...
  if (typeof parserDir !== 'string') {
    throw new _gulpUtil2.PluginError(PLUGIN_NAME, new TypeError('You must provide the directory where to write or read\n     the generated lexing and parsing tools (option \'parserDir\')'));
  }
...
function getMode(options) {
  ...
  if (typeof mode !== 'string' && mode !== undefined) {
    throw new _gulpUtil2.PluginError(PLUGIN_NAME, new TypeError('Bad option: ' + mode));
  }
...
function getRule(options) {
  ...
  if (!rule) {
    throw new _gulpUtil2.PluginError(PLUGIN_NAME, 'Undefined starting rule (option \'rule\')');
  }
